...
def GA_main(input_fp,config):
    ...
    if config["selection"][1]=="TUR":
        toolbox.register("select", config["selection"][0], tournsize=3)
    else:
        toolbox.register("select", config["selection"][0])
    ...
    configs = [
        {
           "cxpb" : 0.2,
           "mutpb" : 0.05,
           "selection" : (tools.selTournament,"TUR")
        },
        {
            "cxpb": 0,
            "mutpb": 0.2,
        ...
        ,
        {
            "cxpb": 0.2,
            "mutpb": 0.05,
            "selection": (tools.selRoulette, "RWS")
        }
        ,
        {
            "cxpb": 0.2,
            "mutpb": 0.05,
            "selection": (tools.selRandom, "RND")
        }

    ]

    #data_sets = [verysmall_dataset, small_dataset, medium_dataset, large_dataset, verylarge_dataset]
    data_sets = [ small_dataset, large_dataset]
    for data_set in data_sets:
        for config in configs:
            nnodes = 17 if data_set==small_dataset else 60
            with open('results\\results_GA_n%s_cross%0.2f_mut%0.2f_sel%s.csv' %
                      (nnodes,config["cxpb"],config["mutpb"],config["selection"][1]), 'wt') as f:
                f.write('run_number;cost;run_time\n')
                costs = []
                runtimes = []
                for i in range(10):
                    cost, runtime = GA_main(data_set,config)
                    costs.append(cost)
        ...
        # with open('results\\results_AC_%s.csv' % os.path.basename(data_set), 'wt') as f:
        #     f.write('run_number;cost;run_time\n')
        #     costs = []
        #     runtimes = []
        #     for i in range(10):
        #         cost, runtime = ACO_main(data_set)
        #         costs.append(cost)
        #         runtimes.append(runtime)
